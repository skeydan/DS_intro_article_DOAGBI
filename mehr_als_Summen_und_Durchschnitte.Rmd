...
Nehmen wir nun an, wir haben Daten wie die folgenden: Es geht um 


```{r, echo=FALSE}
ames <- read_csv("ames_housing_train.csv")
#fit <- lm(SalePrice ~GrLivArea + FullBath + TotRmsAbvGrd + GarageYrBlt + GarageCars + GarageArea,
#          data = ames)
#summary(fit)
ames %>% select(SalePrice,GrLivArea,FullBath,TotRmsAbvGrd,GarageYrBlt,GarageCars,GarageArea) %>% ggpairs()
```
